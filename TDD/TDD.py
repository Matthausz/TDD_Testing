...
#         print(res)
    ...
    #would be very effiecent to parallise however still exponential
    def get_statevector(self,qubits):
        bases=list(itertools.product(*[(0, 1)] * qubits))
        statevector=np.zeros(2**qubits,dtype=np.complex128)
        i=0
        for basis in bases:
            statevector[i]=self.get_amplitude(list(basis))
            i+=1
        return statevector
